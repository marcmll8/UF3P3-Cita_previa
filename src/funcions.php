...
function creaCalendari($mes, $any, $festius = array())
{
    ...
    for ($i = 1; $i < 8; $i++) {
        if ($i < $diai) {
            ...
            if (!in_array($diaActual, $festius)) {
                $calendari = $calendari . "><a href='../public/index.php?r=citahora&m=$diaActual'";
                if ($dia == $avui) {
                    $calendari = $calendari . 'class="avui" ';
                }
            } else {
                $calendari = $calendari . 'class="festa" ';
            } $calendari = $calendari . ">";

            if ($dia <= $diaf) {
                $calendari = $calendari . $dia;
                $dia++;
            }

            if (!in_array($dia, $festius)) {
                $calendari = $calendari . "</a>";
            }
